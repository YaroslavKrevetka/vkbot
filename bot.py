...
import sqlite3
import time
from config import TOKEN, OWNER_ID  # TOKEN — токен сообщества

# ===== VK =====
PEER_ID = 2000000022  # ID твоей беседы
vk_session = vk_api.VkApi(token=TOKEN)
vk = vk_session.get_api()
longpoll = VkBotLongPoll(vk_session, group_id=OWNER_ID)  # group_id = ID сообщества
...
def mute(user_id, minutes):
    cursor.execute(
        ...
        (user_id, int(time.time()) + minutes * 60)
    )
    db.commit()
...
for event in longpoll.listen():
    ...
    if peer_id != PEER_ID:
        continue  # бот реагирует только в нужной беседе
